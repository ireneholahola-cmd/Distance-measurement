...
```python
from trajectory_prediction.trajectory_predictor import TrajectoryPredictor, RiskFieldIntegrator, TrajectoryVisualizer
...
while True:
    ...
    # 集成到风险场
    integrator = RiskFieldIntegrator(predictor)
    integrated_risk = integrator.integrate(tracks, risk_field)
    
    # 绘制轨迹
    frame = visualizer.draw_trajectories(frame, trajectories)
