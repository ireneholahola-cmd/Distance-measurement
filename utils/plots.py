...
from utils.distance import detect_distance_bicycle
from utils.distance import detect_distance_car
from utils.distance import detect_distance_motorcycle
from utils.distance import detect_distance_bus
from utils.distance import detect_distance_truck
...
def draw_speed(img, speed, bbox_xywh, identities, time_person, dis_m):
    flag=''
    for i, j in enumerate(speed):
        for m, n in enumerate(identities):
            if j[1] == n:
                xy = [int(i) for i in bbox_xywh[m]]
                cv2.putText(img, str(j[0]) + 'km/h', (xy[0], xy[1] - 30), cv2.FONT_HERSHEY_PLAIN, 2.3, [0, 0, 255],
                            2)
                if j[0]:
                    t = dis_m/(float(j[0])*0.277777778)  # 将km/h的速度转化为m/s，再除以两车之间的距离，求得时间t
                    if t <= (time_person * 0.5):  # 若时间t比人脑反应后的刹车时间的1/2更短，则判定为高风险，并进行预警
                        flag = "High risk"
                    elif time_person >= t >= (time_person * 0.5):  # 若时间t介于人脑反应后的刹车时间和人脑反应后的刹车时间的1/2之间，则判定为低风险，并进行预警
                        flag = "Low risk"
                    else:  # 若时间t大于人脑反应后的刹车时间，则判定为无风险
                        flag = "No risk"
                break
    return img, flag
